self.onmessage = function(e) {
  ...
  let currentWeek = [];
  dates.forEach((date, i) => {
    currentWeek.push({date, index: i});
    if (dow[i] === 6 || i === numDays - 1) {
  ...
  weeks.forEach(week => {
    const perWorkDays = perDiem.map(p => week.reduce((count, wd) => count + (sched[wd.date].some(as => as.name === p) ? 1 : 0), 0));
    dates.forEach(date => {
      const ri = dates.indexOf(date);
      while (required[ri].B > 0) {
        let dayPerCount = sched[date].filter(as => perDiem.includes(as.name)).length;
        if (dayPerCount >= 4) {
          localErrors.push(`Max 4 per diem reached on ${date}, can't assign more B`);
          break;
        }
        let minShifts = Infinity;
        let bestP = null;
        perDiem.forEach((p, pi) => {
          const totalAssigned = dates.reduce((count, d) => count + (sched[d].some(as => as.name === p) ? 1 : 0), 0);
          if (availPerSet[p].has(date) && !sched[date].some(as => as.name === p) && perWorkDays[pi] < 5 && totalAssigned < perDiemMax[p]) {
            if (totalAssigned < minShifts) {
              minShifts = totalAssigned;
              bestP = p;
            }
          }
        });
        if (bestP) {
          sched[date].push({ name: bestP, role: 'B' });
          required[ri].B--;
          const pi = perDiem.indexOf(bestP);
          perWorkDays[pi]++;
        } else {
          localErrors.push(`Can't assign B on ${date} - not enough per diem available`);
          break;
        }
      }
    });
